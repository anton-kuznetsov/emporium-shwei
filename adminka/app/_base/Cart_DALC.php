...
class Cart_DALC extends DALC {
	...
	public function GetCartById( $id_cart ) {
		...
		$data = $this->SQL_SelectItem( $this->TABLE_NAME, NULL, $id_cart );
	...
	public function GetCartBySession( $session_id ) {
		...
		$items = $this->SQL_SelectList( $this->TABLE_NAME, NULL, " session_id = '".$session_id."' AND status = 0 " );
	...
	public function NewCart( $session_id ) {
		...
		$item = $this->SQL_CreateItem( $this->TABLE_NAME, array( 'session_id' => $session_id ));
